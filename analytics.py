...
def analytics_dashboard():
    ...
    # ---------------- SAFE AUTO-REFRESH (NO CRASH) ----------------
    if st.session_state.get("new_prediction", False):
        st.session_state.new_prediction = False
        st.info("✅ New prediction detected. Refresh the page to see latest analytics.")
    ...
    col2.metric("Benign Cases", int((df["Prediction"] == "benign").sum()))
    col3.metric("Malignant Cases", int((df["Prediction"] == "malignant").sum()))
    col4.metric("Normal Cases", int((df["Prediction"] == "normal").sum()))
    ...
    # =====================================================
    # TREND GRAPH (LAST 30 DAYS + COLOR-CODED DOTS)
    # =====================================================
    ...
    if df_recent.empty:
        trend = pd.Series(0, index=dates.date)
    else:
        trend = df_recent.groupby(df_recent["Time"].dt.date)["Prediction"].count()
        trend = trend.reindex(dates.date, fill_value=0)

    # color-code dots
    colors = []
    for date, val in zip(trend.index, trend.values):
        if date == today:
            colors.append("red")      # today highlighted
        elif val == 0:
            colors.append("grey")     # no predictions
        else:
            colors.append("blue")     # 1 or more predictions

    fig3, ax3 = plt.subplots()
    ax3.plot(trend.index, trend.values, color="blue", linewidth=1)
    ax3.scatter(trend.index, trend.values, color=colors, s=60)  # dots
